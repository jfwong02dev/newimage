		...
		<div class="col-sm-12">
			<div class="panel">
				<div class="panel-heading">
					<span class="panel-title">New Sale</span>
				</div>
					...
					@csrf
									...
									<select multiple="multiple" class="form-control" id="service" name="service[]">
										...
										<option value="{{$service->code}}" 
											{{ ( 
												...
												in_array($service->code, ($faker['service'] ?? []))
												) ? 'selected' : '' 
											}}>{{ $service->name }}</option>
										@endforeach
									</select>
									...
									<select multiple="multiple" class="form-control" id="product" name="product[]">
										...
										<option value="{{$product->code}}" 
											{{ ( 
												...
												in_array($product->code, ($faker['product'] ?? []))
												) ? 'selected' : '' 
											}}>{{ $product->name }}</option>
										@endforeach
									</select>
						...
						<div class="form-group{{ $errors->has('comm') ? ' has-error' : '' }}">
							...
							<div class="col-sm-9">
								...
								<div class="radio">
									<label>
										<input type="radio" name="comm" value="{{$comm}}" class="px" {{ (old('comm') ?? $faker['comm'] ?? '') === $comm ? 'checked' : '' }} />
