		...
		<div class="col-sm-12">
			<div class="panel">
				...
				<div class="panel-body">
					<div class="col-sm-6">
						<form action="{{route('sales.update', $sale->id)}}" method="post" class="form-horizontal" id="edit-sale-form">
						@csrf
						@method('put')
							...
							<div class="form-group">
								<div class="col-sm-offset-3 col-sm-9">
									<button id="update-btn" type="submit" class="btn btn-primary">{{__('translate.button/update')}}</button>
								</div>
							</div>
						</form>
					</div>
	...
	@section('script')
		...
		<script>
			init.push(function () {
				...
				var old_bulkService = <?php echo old('bulkService') ? json_encode(old('bulkService')) : (count(json_decode($sale->service, true)) > 0 ? json_encode(implode(',', json_decode($sale->service, true))) : json_encode('')); ?>;
				var old_bulkProduct = <?php echo old('bulkProduct') ? json_encode(old('bulkProduct')) : (count(json_decode($sale->product, true)) > 0 ? json_encode(implode(',', json_decode($sale->product, true))) : json_encode('')); ?>;
				var bulkServices = {}
				var bulkProducts = {}
